...
function getAdminNameThroughSession($session_username, $session_id) {
    ...
    $statement = $con->prepare("SELECT `firstName`, `lastName` FROM `tbluseraccounts` WHERE `username` = :session_username AND `userID` = :session_id AND `userRole` = 'SupplierAdmin' LIMIT 1;"); 
